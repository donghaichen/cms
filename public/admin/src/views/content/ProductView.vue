                    ...
                    <Row style="margin: 20px">
                        <Col :lg="8" :md="24">
                           <div style="width: 300px">
                            ...
                            <p v-if="albumUpload === false" style="margin: 15px 0;" class="desc">
                                已上传 <Tag color="success">{{albumCount}}</Tag> 张图片，<Button type="error" @click.native="resetAlbum" style="margin: 10px 0" icon="md-close">
                                清空相册继续上传图片
                            </Button>
                            </p>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      {
        this.$axios.get(this.api.content.view + this.contentType + '/' + this.$route.params.id)
          .then((response) => {
            if (response.data.code == 0)
            {
